...
@cubestat_metric("linux")
class amd_gpu_metric(gpu_metric):
    has_amd: bool
    rocml: Optional[Any] = None

    def __init__(self) -> None:
        """Initialize the AMD ROCm GPU metric."""
        self.has_amd = False
        self.n_gpus = 0
        try:
    ...
    def read(self, _context: Dict[str, Any]) -> Dict[str, float]:
        ...
        if not self.has_amd or self.rocml is None:
            return res
        ...
        try:
            ...
            for i in range(device_count):
                try:
                    ...
                    res[f"GPU {i} util %"] = float(gpu_util)
                    total += float(gpu_util)
                    ...
                    if mem_total > 0:
                        ...
                        res[f"GPU {i} vram used %"] = 0.0
                        logger.warning(f"AMD GPU {i} reports zero total memory")

                    self.n_gpus += 1
                except Exception as e:
                    logger.warning(f"Error processing AMD GPU {i} data: {str(e)}")
